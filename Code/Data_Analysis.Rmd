...
CH4.boxplot <- ggplot(data = Poop_CH4, aes(x=Item, y = Value))+
  geom_boxplot()+
...
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Asses"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Cattle, dairy"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Cattle, non-dairy"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Chickens, broilers"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Chickens, layers"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Goats"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Ducks"]) #normally distributed
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Horses"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Mules"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Sheep"]) #not normal
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Swine, breeding"]) #normally dist
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Swine, market"]) #normally dist
shapiro.test(Poop_CH4$Value[Poop_CH4$Item == "Turkeys"]) #not normal
#most of animals are not normally distributed-reject null in all but 3 cases

qqnorm(Poop_CH4$Value); qqline(Poop_CH4$Value)
...
bartlett.test(Poop_CH4$Value ~ Poop_CH4$Item)
...
```{r, anova}
# Format ANOVA as aov
CH4.anova <- aov(data=Poop_CH4, Value ~ Item)
summary(CH4.anova)
...
# Format ANOVA as lm
CH4.anova2 <- lm(data=Poop_CH4, Value ~ Item)
summary(CH4.anova2)
...
CH4.groups.plot <- ggplot(Poop_CH4, aes(x=Item, y = Value))+
  geom_boxplot() + 
  theme(axis.text.x = element_text(angle=45, hjust=1))+
  stat_summary(geom="text", fun=max, vjust=-1, size=3.5,
               label=c("h", "b", "d", "f", "e", "h", "h", "gh", "h", "h", "c", "a", "fh"))
